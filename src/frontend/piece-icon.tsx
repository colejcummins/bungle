...
export const PieceIcon = forwardRef<SVGSVGElement, PieceProps>(
	({ color, type, ...props }, ref) => {
		const PieceComponent = PIECE_MAP[color][type];

		const fillColor = color === Color.White ? "#fafafa" : "#09090b";
		const strokeColor = color === Color.White ? "#09090b" : "#fafafa";

		return <PieceComponent {...props} ref={ref} fillColor={fillColor} strokeColor={strokeColor} />;
	},
);

PieceIcon.displayName = "Piece";

const PIECE_MAP: Record<Color, Record<PieceType, ReturnType<typeof forwardRef<SVGSVGElement, IndividualPieceProps>>>> = {
	[Color.White]: {
